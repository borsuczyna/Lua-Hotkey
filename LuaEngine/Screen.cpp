...
void WGetPixelColor(int x, int y, int& r, int& g, int& b)
{
	HDC dc = GetDC(NULL);
...
int Screen::GetDimensions(lua_State* L)
{
	int w, h;
...
int Screen::GetPixelColor(lua_State* L)
{
	int x = luaL_checkinteger(L, 1);
	int y = luaL_checkinteger(L, 2);

	int r, g, b;
	WGetPixelColor(x, y, r, g, b);
...
int Screen::FindPixelByColor(lua_State* L)
{
	int x = luaL_checkinteger(L, 1);
	int y = luaL_checkinteger(L, 2);
	...
	int var = luaL_checkinteger(L, 8);

	POINT p = PixelSearch(x, y, w, h, RGB(b, g, r), var);
